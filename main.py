...
def send_line_message(flex_msg):
    try:
        ...
        line_user_id = [
            os.environ.get("USER_ID1"),
            os.environ.get("USER_ID2"),
            os.environ.get("USER_ID3"),
        ]
        if not access_token:
...
def generate_flex_message_dict(today_data, yesterday_data):
    ...
    if yesterday_data is None:
        ...
        bubble_dict["body"]["contents"][3]["contents"][0]["contents"][2]["text"] = (
            data1_1
        )
        bubble_dict["body"]["contents"][3]["contents"][0]["contents"][2]["color"] = (
            "#00AA00" if data1_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][1]["contents"][1]["text"] = data2
        bubble_dict["body"]["contents"][3]["contents"][1]["contents"][2]["text"] = (
            data2_1
        )
        bubble_dict["body"]["contents"][3]["contents"][1]["contents"][2]["color"] = (
            "#00AA00" if data2_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][3]["contents"][1]["text"] = data3
        bubble_dict["body"]["contents"][3]["contents"][3]["contents"][2]["text"] = (
            data3_1
        )
        bubble_dict["body"]["contents"][3]["contents"][3]["contents"][2]["color"] = (
            "#00AA00" if data3_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][4]["contents"][1]["text"] = data4
        bubble_dict["body"]["contents"][3]["contents"][4]["contents"][2]["text"] = (
            data4_1
        )
        bubble_dict["body"]["contents"][3]["contents"][4]["contents"][2]["color"] = (
            "#00AA00" if data4_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][6]["contents"][1]["text"] = data5
        bubble_dict["body"]["contents"][3]["contents"][6]["contents"][2]["text"] = (
            data5_1
        )
        bubble_dict["body"]["contents"][3]["contents"][6]["contents"][2]["color"] = (
            "#00AA00" if data5_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][7]["contents"][1]["text"] = data6
        bubble_dict["body"]["contents"][3]["contents"][7]["contents"][2]["text"] = (
            data6_1
        )
        bubble_dict["body"]["contents"][3]["contents"][7]["contents"][2]["color"] = (
            "#00AA00" if data6_1[1] == "-" else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][11]["contents"][1]["text"] = (
            data7
        )
        bubble_dict["body"]["contents"][3]["contents"][11]["contents"][2]["text"] = (
            data7_1
        )
        bubble_dict["body"]["contents"][3]["contents"][11]["contents"][2]["color"] = (
            "#00AA00" if float(data7) < float(data7_1) else "#FF5555"
        )
        bubble_dict["body"]["contents"][3]["contents"][12]["contents"][1]["text"] = (
            data8
        )
        bubble_dict["body"]["contents"][3]["contents"][12]["contents"][2]["text"] = (
            data8_1
        )
        bubble_dict["body"]["contents"][3]["contents"][12]["contents"][2]["color"] = (
            "#00AA00" if float(data8) < float(data8_1) else "#FF5555"
        )
        ...
        bubble_dict["body"]["contents"][3]["contents"][16]["contents"][1]["text"] = (
            amp_max
        )
        bubble_dict["body"]["contents"][3]["contents"][17]["contents"][1]["text"] = (
            amp_min
        )
        bubble_dict["body"]["contents"][3]["contents"][18]["contents"][1]["text"] = (
            amp_avg
        )
        bubble_dict["body"]["contents"][3]["contents"][20]["contents"][1]["text"] = (
            amp_today
        )
        bubble_dict["body"]["contents"][3]["contents"][20]["contents"][1]["color"] = (
            "#00AA00" if float(amp_today) < float(amp_avg) else "#FF5555"
        )

        flex_msg = FlexMessage.from_dict(
            {"type": "flex", "altText": "股市資訊", "contents": bubble_dict}
        )
    return flex_msg
...
def fetch_taiex_futures_data(query_date):
    try:
        ...
        http = urllib3.PoolManager()
        response = http.request(
            "POST", url, fields={"queryType": 1, "doQuery": 1, "queryDate": query_date}
        )
        soup = BeautifulSoup(response.data, "html.parser")
...
def fetch_option_data(query_date):
    try:
        ...
        for i in range(len(data_frame)):
            if (
                data_frame.iloc[i, 1] == "選擇權 小計"
                and data_frame.iloc[i, 2] == "外資"
            ):
                foreign_total_option_open_interest = clean_data_to_int(
                    data_frame.iloc[i, 13]
                )
            if (
                data_frame.iloc[i, 1] == "臺指選擇權"
                and data_frame.iloc[i, 2] == "外資"
            ):
...
def fetch_large_future_data(query_date):
    try:
        ...
        http = urllib3.PoolManager()
        response = http.request(
            "POST", url, fields={"contractId": "TX", "queryDate": query_date}
        )
        soup = BeautifulSoup(response.data, "html.parser")
        table_html = soup.find_all("table")
        data_frame = pd.read_html(StringIO(str(table_html)))[0]
        large_open_interest = clean_data_to_int(
            data_frame.iloc[1, 2]
        ) - clean_data_to_int(data_frame.iloc[1, 6])
        large_total_open_interest = clean_data_to_int(
            data_frame.iloc[1, 4]
        ) - clean_data_to_int(data_frame.iloc[1, 8])
...
if __name__ == "__main__":
    ...
    # 檢查所有資料是否都成功抓取（重試5次後）
    if (
        futures_result is None
        or option_result is None
        or large_result is None
